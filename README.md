...
M2 is complete. The Android builds use JIT with interpreter and AOT disabled, deriving an application-local `libmonosgen-2.0.so` from the project-local .NET runtime pack. The build patches only Mono's two access-decision functions, so ordinary Mod IL that directly references non-public game members follows the same global access policy as the legacy launcher. Harmony still uses the reproducible `Lib.Harmony 2.4.2-junimogate.11` source patch plus its ARM64 cache helper. The SDK runtime pack is never modified; see [`docs/runtime-probe-result.md`](docs/runtime-probe-result.md).
