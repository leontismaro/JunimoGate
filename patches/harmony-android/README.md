...
The current known-good package recipe is `Lib.Harmony 2.4.2-junimogate.64`. Any source or patch modification requires a new `-junimogate.N` version and proportional host/device verification for the changed Android path. Generated source trees, NuGet packages, and per-build provenance remain ignored; do not commit full upstream source snapshots or locally generated packages here.
